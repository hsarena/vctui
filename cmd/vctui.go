...
func init() {
	...
	vctuiCmd.Flags().StringVar(&dcName, "datacenter", os.Getenv("VC_DATACENTER"), "The Datacenter Name")
...
var vctuiCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		vms, err := vctui.VMInventory(c, dcName, true)
		if err != nil {
			log.Fatalf("%s", err.Error())
		}

		if len(vms) == 0 {
			log.Fatalf("No Virtual Machines to display")
		}

		// Escape code to clear screen (https://godoc.org/github.com/pborman/ansi)
		fmt.Printf("\033[2J")
		vctui.MainUI(vms, dcName, c)
		// Escape code to clear screen (tidy up any artifacts generated by the UI hacks)
		fmt.Printf("\033[2J")
